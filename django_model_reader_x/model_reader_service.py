    ...
    def prepare_joins_data(self,lst_column_details):
        ...
        for column_details in lst_column_details:
            ...
            if column_details.get("datatype")=="ForeignKey":
                ...
                app_name = column_details.get("app_name")

                related_app_name = column_details.get("related_app_name")
                related_model_name = column_details.get("related_model_name")
                if related_app_name+"."+related_model_name in self.lst_app_label_model_name:
                    related_field_name = column_details.get("related_field_name")
                    inst_model = self.get_model_by_name(app_name=related_app_name,model_name=related_model_name)
                    if inst_model:
                        if related_field_name in self.get_model_column_names(model=inst_model):
                            join_details.update({"target_model":related_model_name})
                            join_details.update({"related_field":related_field_name})
                            join_details.update({"related_app_name":related_app_name})
                            join_details.update({'type': 'ForeignKey'})
                            join_details.update({"source_model":source_model})
                            join_details.update({"source_app_name":related_app_name})
                            ...
                            if len(lst_primary_fields)>0:
                                ...
                                join_details.update({'type': 'ForeignKey'})
                                join_details.update({"source_model":source_model})
                                join_details.update({"app_name":app_name})
                                lst_joins.append(join_details)
    ...
    def get_all_models(self):
        ...
        for app_config in self.apps_config:
            ...
            for model in models:
                if not self.is_model_to_exclude(model):
                    model_name = self.get_model_name(model=model)
                    model_description = self.get_model_description(model=model)
                    ...
                    model_details = {}
                    model_details.update({"app_name":app_name,"model_name":model_name,"db_table_name":db_table_name,"model_description":model_description})
